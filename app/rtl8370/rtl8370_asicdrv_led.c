...
CONST_T uint16 ForceLedbits[8]= {0x0003,0x000C,0x0030,0x00C0,0x0300,0x0C00,0x3000,0xC000};
...
ret_t rtl8370_setAsicForceLed(uint32 port, uint32 group, uint32 mode)
{
    ...
    retVal = rtl8370_setAsicRegBits(regAddr, ForceLedbits[port&0x7],mode); 	  
...
ret_t rtl8370_getAsicForceLed(uint32 port, uint32 group, uint32* mode)
{
    ...
    return rtl8370_getAsicRegBits(regAddr, ForceLedbits[port&0x7],mode);
}
...
ret_t rtl8370_setAsicForceGroupLed(uint32 groupmask, uint32 mode)
{
    ...
    uint16 bits[3]= {0x0004,0x0010,0x0040};
    /* Invalid input parameter */
